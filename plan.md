...
**D3 — Lifting + a simple 1/ε pole is supported (Case A; N3, planAXpDIV.md), including complex exponents (planCXLIFTDIV.md).** The (n+1)-dim lifted fan and the post-δ pole machinery compose; the divergent lifted sector reconstructs prefactors from `PrefactorBase` and threads a `DomainConstraint`. Complex-B lifting + divergence is supported via `ComplexExponentMode -> "SplitRealImag"` on both the Subtraction and IBP routes (cc_46). Case B (domain–pole coupling), the inline symbolic-ε Subtraction path for SplitRealImag × lift × divergence, its complex Case-B analogue, and `Direct`-mode complex-B lifting are refused with clean `$Failed` messages. *Recommended.*
