...
\paragraph{Divergent complex monomials (Case A vs Case B).} When the lifted integral also carries a $1/\varepsilon$ pole (Section~\ref{sec:liftdiv}), the imaginary monomial weight must \emph{decouple} from the divergent direction. The divergent slot has $\tilde a_k=c_k\varepsilon$; if the phase also gives it an imaginary part $C_k\neq 0$, the pole moves off $\varepsilon=0$ ($c_k\varepsilon+iC_k$) and the real-pole IBP/subtraction assembly cannot resolve it. \emph{Case A} (supported): $C_k=0$---the pole stays at $\varepsilon=0$ and is genuinely complex-\emph{valued}; the per-IBP-piece \texttt{MonomialPhaseLog} is re-flattened from the un-flattened numerator \texttt{SectorData["LiftedMonoPhase"]} divided by each piece's $\alpha_0$ (the Section~\ref{sec:liftdiv} construction, applied to the monomial phase). \emph{Case B} (refused, \texttt{splitdivmono}): $C_k\neq 0$. The guard sums \emph{both} imaginary sources projected on the divergent slot---$\sum_j\mathrm{Im}(B_j)D_{j,k}$ from the dropped tropical monomial factor \emph{and} $\mathrm{Im}(e_{\mathrm{aug}})_k$ from the bare monomial---and aborts cleanly if either is nonzero, rather than emit a shifted (wrong) pole. A practical note for building Case-A fixtures with $\mathrm{Const}_A\neq 0$: align the lift pivot with the variable that \emph{carries} the complex weight while the divergence sits on a different, real variable; pivoting on the divergent variable forces either $\mathrm{Const}_A=0$ or Case~B (the pivot and divergent projections then couple to one parameter).

\paragraph{Refusals (clean \texttt{\$Failed}, never a wrong number).} (i)~\texttt{Direct}-mode lifting of any complex integrand---\texttt{liftcomplex}; use \texttt{SplitRealImag}. (ii)~An \emph{unlifted} complex integrand with a divergent sector on the inline symbolic-$\varepsilon$ Subtraction path (\texttt{Method->"None"/"Subtraction"}, or $\varepsilon$ absent): that path is not phase-aware---the divergent branch of \texttt{ProcessSector} drops \texttt{MonomialPhaseLog} and reattaches $\mathrm{Im}(B)$ only to convergent sectors---so it refuses with \texttt{splitliftdiv}; use \texttt{Method->Automatic/"IBP"} (which routes complex$+$divergent to the phase-aware IBP driver) or the pinned-$\varepsilon$ \texttt{LaurentFromSubtraction} route. (iii)~Case~B above (\texttt{splitdivmono}). The full path is verified against NIntegrate of the complex original---convergent and divergent Case~A at $n=2$, with Case~B refusing---in cross-check~47.
...
\texttt{splitdivmono} & \texttt{IBPProcessSector} & \textbf{[v3 new, planCXLIFTDIV.md\,/\,planAXpDIVv2.md]} Complex Case-B: a nonzero imaginary exponent in the divergent direction shifts the real pole off $\varepsilon=0$. Sums both sources---the dropped monomial factor $\sum_j\mathrm{Im}(B_j)D_{j,k}$ \emph{and} the bare monomial phase $\mathrm{Im}(e_{\mathrm{aug}})_k$ (Section~\ref{sec:liftcomplex}); use the pinned-$\varepsilon$ Subtraction route. \\
