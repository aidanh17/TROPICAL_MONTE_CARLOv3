...
\texttt{liftcomplex} & \texttt{ProcessSectorLifted} & Complex $\tilde{a}$ has no real domain indicator (\texttt{Direct} mode); use \texttt{ComplexExponentMode->"SplitRealImag"} for complex-$B$ lifting (supported, incl.\ with a $1/\varepsilon$ pole). \\
...
\texttt{splitdivmono} & \texttt{IBPProcessSector} & \textbf{[v3 new, planCXLIFTDIV.md]} Complex Case-B analogue: the dropped monomial factor carries a nonzero $\mathrm{Im}(B)$ exponent in the divergent direction ($\sum_j\mathrm{Im}(B_j)D_{j,k}\neq 0$), shifting the real pole; use the pinned-$\varepsilon$ Subtraction route. \\
