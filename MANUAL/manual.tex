...
\begin{description}[style=nextline]
  ...
  \item[Mutually exclusive with divergence:] a sector is plain-convergent, divergent, or lifted---not divergent+lifted.
\end{description}
...
\subsection{\texttt{tropical\_fan.wl}}
...
Routes on \texttt{"Method"} ($\in$ \texttt{\{Automatic, "None", "IBP", "Subtraction"\}}) and \texttt{"Lift"} ($\in$ \texttt{\{None, Automatic, <liftRules>\}}). The 10-step skeleton is shared: workdir $\to$ fan unpack $\to$ per-sector processing $\to$ optional Validate $\to$ codegen $\to$ compile $\to$ run $\to$ readback $\to$ assemble $\to$ error summary.

Thin wrappers for API continuity:
\begin{description}
  \item[\texttt{EvaluateTropicalMCLifted[spec, kin, opts]}] Sets \texttt{"Lift" -> Automatic} and routes through the unified driver.
...
\texttt{"Lift"} & \texttt{None} & \texttt{None|Automatic|<rules>}; mutually exclusive with divergence \\
...
For a divergent sector (\texttt{IsDivergent -> True}) the association contains the pre-flattening data only (no \texttt{FlattenedPolys}; \texttt{Prefactor} holds just $|\det(M)|$). The lifted path consumes this pre-flattened form before its own delta resolution.
...
\texttt{liftdivergent} & \texttt{ProcessSectorLifted} & Selected pivot leaves some $\mathrm{Re}(\tilde{a}_j)\leq 0$. \\
...
\item \textbf{Lifting and divergence are mutually exclusive.}
A sector is plain-convergent, divergent, or lifted---not divergent+lifted. The two use different fans ($n$-dim vs $(n{+}1)$-dim). Combining them is documented future work.
...
\subsection{V3 Cross-Check Suite (TEST/, 43 named checks)}
...
43 & Exactness guard: \texttt{FreeQ[symbolicOutput, \_Real]} before the \texttt{MmaToC} boundary. \\
\bottomrule
\end{tabular}
\end{center}
