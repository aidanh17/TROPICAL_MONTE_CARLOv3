...
\begin{description}[style=nextline]
  ...
  \item[Composes with divergence (Case A):] a lifted sector may \emph{also} carry a single $1/\varepsilon$ pole. After the lifting $\delta$-resolution collapses the auxiliary dimension, a lifted sector is structurally an ordinary $n$-dimensional \texttt{SectorData} whose pole lives in one of the surviving effective exponents $\tilde a_j$---the object Stage~3a already consumes. \texttt{ProcessSectorLifted} is $\varepsilon$-aware and routes through the divergence machinery (Section~\ref{sec:liftdiv}). Complex polynomial exponents $B$ compose too, via \texttt{ComplexExponentMode->"SplitRealImag"} on both routes (planCXLIFTDIV.md, cross-check~46), and so do complex \emph{monomial} exponents $A$ (the principal-series bubble at imaginary $\mu$, planAXpDIVv2.md, cross-check~47): \texttt{SplitRealImag} splits \emph{both} families---$\mathrm{Re}(A),\mathrm{Re}(B)$ build the real measure and domain map, $\mathrm{Im}(A),\mathrm{Im}(B)$ ride the oscillatory phase ($\mathrm{Im}(A)$ via a bare-monomial phase $\exp(i(\mathrm{Const}_A+\sum_a (\mathrm{Im}(A)\!\cdot\!M)_a/\alpha^0_a \log y'_a))$, the monomial twin of \texttt{MonoFactorLog}). A nonzero imaginary exponent in the divergent direction ($\mathrm{Im}(B)\!\cdot\!D$ or $\mathrm{Im}(A)\!\cdot\!M$) is now \emph{supported} on the IBP route as the \textbf{off-axis} case ($s_k=c_k\varepsilon+i\theta_k$ regular at $\varepsilon=0$; cone finite, pole $0$; planIBPCX.md), and IBP \emph{batches} over kinematics. Refused cases: the divergent variable coupling to the lifted domain face (Case~B); \texttt{SplitRealImag}$\times$lift$\times$divergence on the \emph{inline symbolic-}$\varepsilon$ Subtraction path; an \emph{$\varepsilon$-dependent} imaginary exponent on the divergent direction (\texttt{splitdivmono}); and \texttt{Direct}-mode complex-exponent lifting.
\end{description}
...
\subsection{Auxiliary Lifting with Complex Exponents (\texttt{SplitRealImag}~$\times$~lift)}\label{sec:liftcomplex}

So far the lifting machinery (Section~\ref{sec:lifting}) and the complex-exponent phase (Section~\ref{sec:complexcodegen}) were described separately. This subsection is how they compose---how to implement the auxiliary uplift when the integrand is genuinely complex (\texttt{planAXpDIVv2.md}, \texttt{planCXLIFTDIV.md}).

\paragraph{The obstruction.} Lifting is built on \emph{real} machinery: the VEGAS importance measure, the convergence gate $\mathrm{Re}[\tilde a]>0$, the pivot ranking, and the domain indicator $\log y_p^*\leq 0$ all require real effective exponents. A complex integrand breaks this---the post-$\delta$ effective exponents $\tilde a_j$ inherit the imaginary weight and become genuinely complex, so there is no real domain face to test. The imaginary part can sit in either family: a complex \emph{polynomial} exponent $B_j$, or a complex \emph{monomial} exponent $A_i$ (the principal-series ``bubble at imaginary $\mu$''---the first lifted integrand whose monomial exponents are complex). In \texttt{Direct} mode the lift aborts on every cone with \texttt{liftcomplex}.

\paragraph{Realify before lifting, restore the phase after.} \texttt{ComplexExponentMode->"SplitRealImag"} feeds the lift/fan/divergence machinery a \emph{purely real} spec and re-attaches the imaginary part as an oscillatory phase in the C++ integrand. Both families are realified up front:
\begin{itemize}\itemsep2pt
  \item $B_j\to\mathrm{Re}(B_j)$ --- decompose on $\mathrm{Re}(B)$ (Section~\ref{sec:complexcodegen});
  \item $A_i\to A_i-i\,\mathrm{Im}(A_i)$ --- \textbf{subtract} the imaginary part; do \emph{not} apply $\mathrm{Re}[\cdot]$.
\end{itemize}
The subtraction (not $\mathrm{Re}$) is load-bearing. The real regulator $\varepsilon$ lives \emph{inside} the monomial exponent ($x_1^{-1+\varepsilon}$ is how the $1/\varepsilon$ pole enters), so $\mathrm{Re}[-1+\varepsilon+ig]$ would wrap the regulator in $\mathrm{Re}[\varepsilon]$ and leak it into the double-typed C++ domain indicator, whereas $(-1+\varepsilon+ig)-ig=-1+\varepsilon$ keeps $\varepsilon$ exact. It must also be done on the \emph{symbolic} spec \emph{before} any $\varepsilon$-pinning: subtracting after pinning $\varepsilon$ to a float would leave \texttt{Complex[float, 0.]} (a machine-zero imaginary part that does not reduce to a real), re-poisoning the indicator. Internally \texttt{imagMonoInfo} extracts the $\varepsilon$-free $\mathrm{Im}(A)$ with the declared kinematic symbols assumed real (so a real symbolic exponent is \emph{not} mistaken for an imaginary one), and \texttt{realifyMonoA} performs the subtraction; both run identically at every detection/realify site so the real path stays byte-identical (\#25).

\paragraph{The monomial phase from $\mathrm{Im}(A)$.} Just as $\mathrm{Im}(B)$ rides back as $\exp(i\,\mathrm{Im}(B)(\log P+\texttt{MonoFactorLog}))$, the subtracted $\mathrm{Im}(A)$ rides back as a \emph{bare-monomial} oscillatory phase---the monomial twin of \texttt{MonoFactorLog}. The lift's pivot substitution carries the imaginary monomial weight through the $\delta$-resolution. With the augmented imaginary weight $\mathrm{Im}(e_{\mathrm{aug}})=\mathrm{Im}(A_{\mathrm{aug}})\cdot M$ ($M$ the augmented ray matrix) and the pivot rule $\log Y_p=(\log z_0-\sum_j m_{\mathrm{other},j}\log y_j)/m_p$, the flattened phase is $\exp\!\bigl(i\,(\mathrm{Const}_A+\sum_j\mathrm{Coeffs}_j\log y'_j)\bigr)$ with
\begin{equation}\label{eq:monophase}
  \mathrm{Const}_A=\frac{\mathrm{Im}(e_{\mathrm{aug}})_p}{m_p}\,\log z_0,
  \qquad
  \mathrm{Coeffs}_j=\frac{\mathrm{Im}(e_{\mathrm{aug}})_{r(j)}-\mathrm{Im}(e_{\mathrm{aug}})_p\,m_{\mathrm{other},j}/m_p}{\tilde a_j}.
\end{equation}
Stored as \texttt{SectorData["MonomialPhaseLog"]=\{Const,Coeffs\}}---structurally identical to \texttt{MonoFactorLog}, but with the single vector $\mathrm{Im}(e_{\mathrm{aug}})$ in place of the dropped factor's $d_k$ and \emph{no} re-clearing term (the bare monomial is not a polynomial). The $\log z_0$ in $\mathrm{Const}_A$ is the term genuinely new to \emph{lifting} a complex monomial: an unlifted complex monomial has $\mathrm{Const}=0$ (Section~\ref{sec:complexcodegen}); lifting threads the anchor scale $z_0$ into the phase through the pivot. The codegen skips the phase term entirely when $\mathrm{Const}_A$ and all $\mathrm{Coeffs}$ vanish (mirroring the per-term $\mathrm{Im}(B)$ skip).

\paragraph{Divergent complex monomials: on-axis ($\theta_k=0$) vs off-axis ($\theta_k\neq 0$).} When the lifted integral also carries a $1/\varepsilon$ pole (Section~\ref{sec:liftdiv}), the imaginary monomial weight projected on the divergent slot is $\theta_k=\sum_j\mathrm{Im}(B_j)D_{j,k}+\mathrm{Im}(e_{\mathrm{aug}})_k$ (dropped monomial factor $+$ bare monomial). The divergent endpoint exponent is then $s_k=c_k\varepsilon+i\theta_k$, and there are two regimes---\emph{both supported on the IBP route} (planIBPCX.md). \emph{On-axis} ($\theta_k=0$): the $1/\varepsilon$ pole stays at $\varepsilon=0$ and is genuinely complex-\emph{valued}; the per-IBP-piece \texttt{MonomialPhaseLog} is re-flattened from the un-flattened numerator \texttt{SectorData["LiftedMonoPhase"]} divided by each piece's $\alpha_0$ (the Section~\ref{sec:liftdiv} construction, applied to the monomial phase). \emph{Off-axis} ($\theta_k\neq 0$): $s_k$ is \emph{regular} at $\varepsilon=0$ (the ``pole'' sits at $\varepsilon=-i\theta_k/c_k$, off the real axis), so the cone is \textbf{finite}---pole $0$, value $\sim 1/(i\theta_k)$. IBP is the \emph{unique} route that resolves it numerically: it \emph{raises} the divergent exponent before flattening, so the bulk $\theta$-phase coefficient is $\theta/\alpha_0^{\text{raised}}=O(\theta)$, bounded (the inline-Subtraction and pinned-$\varepsilon$ routes flatten by the \emph{vanishing} exponent and oscillate too fast). The driver carries $\theta_k$ as \texttt{SectorData["ImagPole"]} (exact $\mathrm{Im}[\cdot]$, $0$ for on-axis $\Rightarrow$ byte/numerically identical real path) and branches the Laurent assembly accordingly (cross-checks 47\,(C), 50). A practical note for building on-axis fixtures with $\mathrm{Const}_A\neq 0$: align the lift pivot with the variable that \emph{carries} the complex weight while the divergence sits on a different, real variable; pivoting on the divergent variable forces either $\mathrm{Const}_A=0$ or off-axis (the pivot and divergent projections then couple to one parameter).

\paragraph{One pole $+$ $N$ off-axis in the same cone (planIBPMULTIDIV.md).} A cone whose soft rays mix a genuine pole ($\mathrm{Re}(\alpha^0)=0\wedge\theta=0$) with $N\geq1$ off-axis directions ($\mathrm{Re}(\alpha^0)=0\wedge\theta\neq0$) was formerly miscounted as a nested divergence and refused (\texttt{nestedIBP} counted every $\mathrm{Re}\leq0$ direction). Directions are now classified by $(\mathrm{Re},\theta)$ (\texttt{ibpDivClass}, decided symbolically via \texttt{PossibleZeroQ}); only genuine poles count toward the refusal. The sector is assembled (\emph{unlifted}) by the full $2^{N+1}$-corner iterated IBP (\texttt{IBPBuildCorners}): each divergent direction is peeled into a \emph{boundary} corner ($y_d{=}1$, drop the coordinate, sign $+$) or a \emph{bulk} corner (raise $\alpha_d$ via \texttt{IBPExpandOneVariable}, sign $-$). The global prefactor $[1/(c_k\varepsilon)]\prod_j[1/(i\theta_j)]$ factors out of the corner sum---the pole $1/(c_k\varepsilon)$ is the \emph{only} $\varepsilon$-singular factor, so the Laurent stays orders $-1$ and $0$ (no $1/\varepsilon^2$). With $\mathrm{NB}=\sum_{\text{corners}}\mathrm{sign}\cdot\mathrm{base}$, $\mathrm{NL}=\sum\mathrm{sign}\cdot\mathrm{log}$, $K_0=\prod_j 1/(i\theta_j)$ and $K_1=K_0\sum_j(-c_j/(i\theta_j))$: $\text{pole}=K_0\,\mathrm{NB}/c_k$ and $\text{finite}=(K_0\,\mathrm{NL}+(K_1-r_kK_0)\mathrm{NB})/c_k+\texttt{DLogPrefactor}\cdot\text{pole}$; for $|D|=1$ this reduces \emph{exactly} to the single-pole / single-off-axis formulas (the existing paths are untouched when no off-axis direction is present, so \#25 holds). Verified against the closed-form Dirichlet oracle for $N=1$ (cross-check~51) and $N=2$. The \emph{lifted} co-located case is still refused at \texttt{ProcessSectorLifted} (\texttt{liftnopivot}: its pivot admittance counts realified $\mathrm{Re}\leq0$ directions, not poles); the off-axis-aware lifted pivot admittance is future work (cross-check~52\,(B)).

\paragraph{Refusals (clean \texttt{\$Failed}, never a wrong number).} (i)~\texttt{Direct}-mode lifting of any complex integrand---\texttt{liftcomplex}; use \texttt{SplitRealImag}. (ii)~An \emph{unlifted} complex integrand with a divergent sector on the inline symbolic-$\varepsilon$ Subtraction path (\texttt{Method->"None"/"Subtraction"}, or $\varepsilon$ absent): that path is not phase-aware---the divergent branch of \texttt{ProcessSector} drops \texttt{MonomialPhaseLog} and reattaches $\mathrm{Im}(B)$ only to convergent sectors---so it refuses with \texttt{splitliftdiv}; use \texttt{Method->Automatic/"IBP"} (which routes complex$+$divergent to the phase-aware IBP driver) or the pinned-$\varepsilon$ \texttt{LaurentFromSubtraction} route. (iii)~an \emph{$\varepsilon$-dependent} imaginary exponent on the divergent direction (\texttt{splitdivmono}, planIBPCX.md~\S8)---a \emph{constant} off-axis $\theta_k\neq 0$ is supported (above), only an $\theta_k\sim1/\varepsilon$ residue is refused. The full path is verified against NIntegrate of the complex original---convergent, divergent on-axis (genuinely complex pole), and divergent off-axis (pole $0$, finite)---in cross-checks~47, 49, 50.
...
\paragraph{Batch mode (\texttt{"Batch" -> True}).} VEGAS only. Shares one low-discrepancy sample set across a chunk of kinematic points (up to \texttt{CubaMaxComp}/2 points per chunk for complex integrands; hard ceiling to avoid CUBA segfault). Supported on the convergent/lifted-convergent path \emph{and} the divergent/IBP path: on the IBP path it runs chunked-ncomp VEGAS over the IBP \emph{function} table (one VEGAS call per (function, kp-chunk) sharing samples across the chunk), preserving the $(1+n_{\mathrm{ibp}})$-row-per-kp output contract so the driver combine is unchanged (planIBPCX.md~\S4; cross-check~50 gates batched IBP $=$ per-kp IBP). Typically 30--60$\times$ wall-clock speedup on large scans.
...
\texttt{"MonoFactorLog"} & \textbf{[v3 new]} \texttt{\{Const,Coeffs\}} for the $\mathrm{Im}(B)$ oscillatory phase from the dropped tropical monomial factor (Section~\ref{sec:complexcodegen}); \texttt{None} on real sectors. \\
\texttt{"MonomialPhaseLog"} & \textbf{[v3 new]} \texttt{\{Const,Coeffs\}} for the bare-monomial $\mathrm{Im}(A)$ phase, Eq.~\eqref{eq:monophase}; \texttt{Const} carries the $\log z_0$ anchor term. \texttt{None} on real-$A$ sectors (Section~\ref{sec:liftcomplex}). \\
\texttt{"LiftedMonoFactor"}, \texttt{"LiftedMonoPhase"} & \textbf{[v3 new]} Un-flattened numerators of the two phases, re-flattened per IBP piece by its own $\alpha_0$ on the divergent route; \texttt{None} on real / unlifted sectors. \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{nestedIBP} & \texttt{IBPProcessSector} & $>1$ genuine $1/\varepsilon$ pole (each $\mathrm{Re}(\alpha^0)=0\wedge\theta=0$); a real $1/\varepsilon^{d\geq2}$, refused. \textbf{[v3, planIBPMULTIDIV.md]} Co-located \emph{off-axis} directions ($\theta\neq0$) are finite and no longer counted---one pole $+\,N$ off-axis is supported (\texttt{ibpDivClass}/\texttt{IBPBuildCorners}; cross-check~51). \\
...
\texttt{liftcomplex} & \texttt{ProcessSectorLifted} & Complex $\tilde{a}$ has no real domain indicator (\texttt{Direct} mode); use \texttt{ComplexExponentMode->"SplitRealImag"} for complex-exponent lifting---complex $A$ \emph{or} $B$ (supported, incl.\ with a $1/\varepsilon$ pole; Section~\ref{sec:liftcomplex}). \\
...
\texttt{splitliftdiv} & driver & \textbf{[v3 new]} \texttt{SplitRealImag}$\times$lift$\times$divergence on the \emph{inline symbolic-}$\varepsilon$ Subtraction path (\texttt{Method->"None"/"Subtraction"}); use \texttt{Method->Automatic/"IBP"} or pinned-$\varepsilon$ \texttt{LaurentFromSubtraction} (both supported, planCXLIFTDIV.md). \\
\texttt{splitdivmono} & \texttt{IBPProcessSector} & \textbf{[v3 new, planIBPCX.md]} The divergent direction's imaginary exponent $\theta_k$ is \emph{$\varepsilon$-dependent} (scales $\sim 1/\varepsilon$), reintroducing fast oscillation. A \emph{constant} $\theta_k\neq 0$ is no longer refused---it is the \textbf{off-axis} case now supported on the IBP route ($s_k=c_k\varepsilon+i\theta_k$ regular at $\varepsilon=0$; cone finite, pole $0$; Section~\ref{sec:liftcomplex}, cross-checks 47\,(C), 50). The guard now fires only for the $\varepsilon$-dependent residue (planIBPCX.md~\S8). \\
\texttt{offaxispow} & \texttt{IBPProcessSector} & \textbf{[v3 new, planIBPMULTIDIV.md]} A \emph{power}-divergent off-axis direction ($\mathrm{Re}(\alpha^0)<0\wedge\theta\neq0$): one IBP step does not raise it to $\mathrm{Re}>0$. The supported off-axis case is logarithmic ($\mathrm{Re}(\alpha^0)=0$). \\
\texttt{ibpresidual} & \texttt{IBPProcessSector} & \textbf{[v3 new, planIBPMULTIDIV.md]} A term retains a genuine on-axis ($\theta=0$) divergence after IBP reduction (an unreduced residual / higher-order pole); refused (replaces a latent non-propagating \texttt{Return[\$Failed]} inside the term \texttt{Table}). \\
\texttt{ibpvalmultidiv} & \texttt{ValidateIBP} & \textbf{[v3 new, planIBPMULTIDIV.md]} The finite-$\varepsilon$ single-pole reconstruction oracle does not cover a one-pole $+\,N$-off-axis MultiDiv sector (validated instead against the closed-form Dirichlet oracle, cross-checks 51/52); returns \texttt{\$Failed} rather than indexing the absent \texttt{IBPTerms}. \\
...
A lifted sector may also carry a single $1/\varepsilon$ pole (Section~\ref{sec:liftdiv}): after the $\delta$-resolution the pole sits in a surviving effective exponent $\tilde a_k$, which the IBP/subtraction machinery consumes; the sector carries a \texttt{PrefactorBase} and a \texttt{DomainConstraint}, and both the \texttt{LaurentFromSubtraction} (robust/universal) and \texttt{Method->"IBP"} routes resolve it (verified across $n=2,3,4$ on non-trivial polynomials, cross-checks 41-C and 45). \textbf{Complex polynomial exponents $B$ compose with lifting~$+$~a $1/\varepsilon$ pole} via \texttt{ComplexExponentMode->"SplitRealImag"} on \emph{both} routes (planCXLIFTDIV.md, cross-check~46): the sector is decomposed on $\mathrm{Re}(B)$ (real measure, real divergence/domain machinery) and $\mathrm{Im}(B)$ rides as an oscillatory phase $\exp(i\,\mathrm{Im}(B)(\log P + \texttt{MonoFactorLog}))$. On the IBP route the per-IBP-piece \texttt{MonoFactorLog} is re-derived (un-flattened numerators divided by each piece's $\alpha_0$) \emph{and} the brought-down IBP coefficient carries the full complex $B_j$ (the $i\,\mathrm{Im}(B_j)$ phase-derivative term); the IBP and Subtraction complex poles agree to $\sim 10^{-4}$ and match NIntegrate of the complex original ($n=2,3$). \textbf{Complex monomial exponents $A$ compose the same way} (Section~\ref{sec:liftcomplex}, planAXpDIVv2.md, cross-check~47): $A$ is realified by \emph{subtracting} $i\,\mathrm{Im}(A)$ (not $\mathrm{Re}$, which would leak the real regulator $\varepsilon$ in the monomial exponent) and $\mathrm{Im}(A)$ rides back as a bare-monomial phase \texttt{MonomialPhaseLog} whose $\mathrm{Const}_A=(\mathrm{Im}(e_{\mathrm{aug}})_p/m_p)\log z_0$ carries the lift anchor; Case~A requires the imaginary weight to decouple from the divergent slot. The single-pole-per-sector limit (item~1) still applies. Refused with a clean \texttt{\$Failed} (never a wrong number): (i)~\textbf{Case B}---the divergent variable couples to the lifted domain face ($\mathrm{ic}_k\neq 0$) for every admissible pivot (\texttt{liftdivdomain}; the Case-A-first pivot ranking makes this rare); (ii)~\texttt{SplitRealImag}$\times$lift$\times$divergence on the \emph{inline symbolic-}$\varepsilon$ Subtraction path, \emph{lifted or unlifted} (\texttt{splitliftdiv}; use \texttt{Method->Automatic/"IBP"} or pinned-$\varepsilon$ \texttt{LaurentFromSubtraction}); (iii)~an \emph{$\varepsilon$-dependent} imaginary exponent on the divergent direction (\texttt{splitdivmono})---a \emph{constant} off-axis $\theta_k\neq 0$ ($\mathrm{Im}(B)\!\cdot\!D$ or $\mathrm{Im}(A)\!\cdot\!M$) is now \emph{supported} on the IBP route (finite cone, pole $0$; planIBPCX.md, Section~\ref{sec:liftcomplex}); (iv)~\texttt{Direct}-mode complex lifting, $A$ or $B$ (\texttt{liftcomplex}---a complex $\tilde a$ has no real domain indicator; use \texttt{SplitRealImag}). When non-trivial polynomials produce \texttt{HasConstantTerm=False} sectors the per-sector IBP pole undercounts (item~3 mechanism); the Subtraction route is robust there and is the recommended route.
...
46 & Complex-$B$ $\times$ lift $\times$ divergence (\texttt{SplitRealImag}): IBP vs Subtraction vs \texttt{NIntegrate} complex Laurent, $n=2,3$; complex poles agree to $\sim10^{-4}$. \\
47 & Complex-\emph{monomial}-$A$ $\times$ lift $\times$ divergence (\texttt{SplitRealImag}): (A)~convergent vs \texttt{NIntegrate}; (B)~Case-A genuinely complex pole, IBP$\leftrightarrow$Subtraction$\leftrightarrow$\texttt{NIntegrate}; (C)~\textbf{off-axis} ($\theta\neq0$) IBP (pole$\approx0$, finite) vs \texttt{NIntegrate} (planIBPCX.md). \\
48 & Complex exponents $\times$ divergence, \emph{unlifted}: monomial-$A$ and polynomial-$B$, \texttt{Direct}$=$\texttt{SplitRealImag}$=$exact $\Gamma$-Laurent, IBP$\leftrightarrow$Subtraction agree (isolates the phase machinery from lifting). \\
49 & \textbf{F1 genuine complex pole} ($\theta=0$): 4-way agreement IBP / Subtraction / \texttt{NIntegrate} / exact $\Gamma$-Laurent on pole \emph{and} finite; IBP$\leftrightarrow$Subtraction to $\sim10^{-5}$ (planIBPCX.md~\S5.1). \\
50 & \textbf{F2 off-axis} ($\theta\neq0$) divergent directions: IBP (pole$\approx0$, finite) vs exact $\Gamma$-Laurent ($\sim10^{-5}$) \& \texttt{NIntegrate} $+$ reseed; \textbf{batched IBP $=$ per-kp IBP} (planIBPCX.md). \\
\bottomrule
\end{tabular}
\end{center}
